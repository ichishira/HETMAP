...
def ret_masked_img(hist_img, mask_img, level):

    tum_mask = np.where((mask_img == 3) | (mask_img == 12), 1, 0)
...
class Dataset_for_sample_with_mask(Dataset):
    def __init__(self, wsi, mask_wsi_np, coords, transform, level, patch_size):
        self.wsi = wsi
        self.mask_wsi_np = mask_wsi_np
        self.coords = coords
        self.level = level
    ...
    def __getitem__(self, idx):
        ...
        mask_img = self.mask_wsi_np[patch_coords[1]:patch_coords[1]+self.patch_size, patch_coords[0]:patch_coords[0]+self.patch_size]

        patch_img, masked_patch_img = ret_masked_img(hist_img, mask_img, self.level)
...
def main():
    ...
    if level ==20:
        ...
        mask_path = f"/wsi/analysis/TCGA_TMA_analysis/data/paget_new/{cancer_type}/{sample_id}/mask.png"
        h5_file_path = f"/data/shirasuna/work/hetero_path/TCGA_wsi_patch/patched_{cancer_type}_mag20_2/patches/{sample_id}.h5"
    ...
    mask_wsi = Image.open(mask_path)
    mask_wsi_np = np.array(mask_wsi)
    
    wsi_width, wsi_height = hist_wsi.dimensions

    if mask_wsi_np.shape[0] == wsi_height:
        pass 
    elif abs(mask_wsi_np.shape[0] - wsi_height / 2) < 2:
        mask_wsi_np = np.repeat(np.repeat(mask_wsi_np, 2, axis=0), 2, axis=1)
    else:
        raise ValueError(f"mask size is not compatible with slide size")
    ...
    def process_patch(coord):
        x, y = coord
        slide_width, slide_height = hist_wsi.dimensions
    
        # スライドの範囲内に収まっているかを確認
        if x + patch_size > slide_width or y + patch_size > slide_height:
            return None  # 範囲外ならスキップ
    
        mask_img = mask_wsi_np[y:y+patch_size, x:x+patch_size]
        tum_mask = np.where((mask_img == 3) | (mask_img == 12), 1, 0)
    ...
    test_dataset = Dataset_for_sample_with_mask(hist_wsi, mask_wsi_np, coords_with_tumor, transform, level, patch_size)
    test_dataloader = DataLoader(test_dataset, num_workers=30, batch_size=100, shuffle=False)
